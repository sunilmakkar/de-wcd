...
#create container volume mounting to host dir so that the python scrip can read and write data from the host
VOLUME /app/input 
VOLUME /app/output 
...
#make sure the container keep running 
CMD tail -f /dev/null

#run the following command in docker exec to run python script. python3:
